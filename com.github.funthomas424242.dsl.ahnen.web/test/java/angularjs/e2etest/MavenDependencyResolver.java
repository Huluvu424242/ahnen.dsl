...
public class MavenDependencyResolver {
    ...
    private static final String LOCAL_MAVEN_REPO = System
        .getProperty("maven.repo.local") != null
    ...
    public static File resolve(final String qualifiedArtifactId) {
        final String[] segments = qualifiedArtifactId.split(":");
        return resolve(segments[GROUP_ID_INDEX], segments[ARTIFACT_ID_INDEX],
                segments[VERSION_INDEX], segments.length > CLASSIFIER_INDEX
                        ? segments[CLASSIFIER_INDEX] : null,
                "jar");
    }

    public static File[] resolve(final String[] qualifiedArtifactIds) {
        final File[] files = new File[qualifiedArtifactIds.length];
